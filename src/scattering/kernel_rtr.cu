										 ...
										 float const *re_x, float const *im_x, float *re_Wx, float *im_Wx)
{
	...
	if ((s1!=s2)&&s1<=NS)
	{
		...
		for (int tau1=1; tau1<=2; tau1++){ // matrix vector product W*x -- remember that W is the transpose of A (l1<->l2 etc.)
				...
				for (int m1=-l1; m1<=l1; m1++){
					...
					for (int m2=-LMAX; m2<=LMAX; m2++){
						for (int tau2=1; tau2<=2; tau2++){
							for (int l2=max(1,abs(m2)); l2<=LMAX; l2++){
								int n2 = (tau2-1)*LMAX*(LMAX+2)+(l2-1)*(l2+1)+l2+1+m2;
								int xIdx = (n2-1)*NS+s2-1;
								float re_xTmp = re_x[xIdx];
								float im_xTmp = im_x[xIdx];
								
								for (int m=-min(l1,l2); m<=min(l1,l2); m++){
									// rotation 
									float real_D[LMAX+1];
									float imag_D[LMAX+1];
									wignerD(m2, m, phi, cosTheta, 0, real_D, imag_D);  // consider precomputing
									float real_D1 = real_D[l2];
									float imag_D1 = imag_D[l2];
									
									wignerD(m, m1, 0, cosTheta, -phi, real_D, imag_D);
									float real_D2 = real_D[l1] * powf(-1.0f, m+m1);  // [Doicu, B.41]
									float imag_D2 = imag_D[l1] * powf(-1.0f, m+m1);
